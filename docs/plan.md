...
For Vast containers without `/dev/net/tun`, use Tailscale userspace networking
and local TCP forwards inside the container. The deploy path should expose local
Postgres endpoints on `127.0.0.1:15432` and `127.0.0.1:15433`, forwarding to the
external HAProxy listeners over Tailscale.
...
Live readiness update from 2026-06-01:

```text
Vast app supervisor is running FastAPI on 127.0.0.1:8002.
The Qwen3.6 27B FP8 vLLM process is reused at 127.0.0.1:18000.
App Redis runs on 127.0.0.1:6380 to avoid Vast/Ray Redis on 6379.
model.ansuman.yral.com reaches FastAPI through Cloudflare -> Caddy :8000 -> 127.0.0.1:8002.
/health, /docs, /openapi.json, /v1/models, and authenticated /v1/chat/completions are verified.
/metrics is blocked publicly.
Prometheus/Grafana and SSE tunnel buffering checks are still deferred.
```
...
3. Set up or verify deploy-managed Tailscale userspace networking between the GPU container and your servers.
...
Inside the Vast container, the service should use local deploy-managed forwards
on `127.0.0.1:15432` and `127.0.0.1:15433` because Tailscale userspace mode does
not provide transparent kernel routes to tailnet addresses.
